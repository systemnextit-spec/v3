    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-2 px-4 flex justify-around items-center md:hidden z-50 pb-safe h-[56px]">
        ...
        <button onClick={onCartClick} className={`flex flex-col items-center gap-1 transition-all relative ${activeTab === 'cart' ? 'text-theme-primary' : 'text-gray-400 hover:text-gray-600'}`}>
            <div className="relative">
                ...
                {(cartCount ?? 0) > 0 && <span className="absolute -top-1.5 -right-1.5 bg-theme-primary text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{cartCount}</span>}
            </div>
...
const BottomNavStyle4: React.FC<MobileBottomNavProps> = ({ 
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-white via-white to-white/90 backdrop-blur-lg border-t border-gray-100 py-2 px-6 flex justify-between items-end md:hidden z-50 pb-safe h-[68px]">
